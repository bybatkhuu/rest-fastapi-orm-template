...
from sqlalchemy import Engine, URL, MetaData
...
@validate_call(config={"arbitrary_types_allowed": True})
async def async_create_structure(async_engine: AsyncEngine, metadata: MetaData) -> None:
    ...
    try:
        async with async_engine.begin() as _connection:
            await _connection.run_sync(metadata.create_all)
...
@validate_call(config={"arbitrary_types_allowed": True})
def create_structure(engine: Engine, metadata: MetaData) -> None:
    """Initialize and create database structure.
    ...
    try:
        with engine.begin() as _connection:
            metadata.create_all(bind=_connection)
