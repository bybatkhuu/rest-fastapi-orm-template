...
def _get_cls_prefix(cls: type) -> str:
    """Get prefix for ORM object primary key."""

    _prefix = ""
    _words = [
        _word
        for _word in camel_to_snake(cls.__name__).removesuffix("_orm").split("_")
        if _word
    ]
    ...
    if not _words:
        ...
        _prefix = _words[0][:3].ljust(3, "x")
    elif len(_words) == 2:
        first, second = _words
        _prefix = first[:2].ljust(2, "x") + second[:1].ljust(1, "x")
    else:
        _prefix = "".join(_word[0] for _word in _words[:3]).ljust(3, "x")
...
class IdStrMixin:
    ...
    def id(cls) -> Mapped[str]:
        _prefix = _get_cls_prefix(cls=cls)
        _id_column = mapped_column(
            String(64),
            primary_key=True,
            default=lambda: gen_unique_id(prefix=_prefix),
            sort_order=-100,
        )
        return _id_column
...
class BaseMixin(TimestampMixin, IdStrMixin):
    ...
    def to_dict(
        self,
        excludes: Collection[str] | list[str] | set[str] | None = None,
        load_relations: Collection[str] | list[str] | set[str] | None = None,
    ) -> dict[str, Any]:
        """Convert ORM object to dictionary.

        Args:
            excludes       (Collection[str] | list[str] | set[str] | None, optional): List or set of attributes to
                                                                                        exclude. Defaults to None.
            load_relations (Collection[str] | list[str] | set[str] | None, optional): List or set of relationships to
    ...
    def to_dict_list(
        cls,
        orm_objects: Sequence[DeclarativeBase],
        excludes: Collection[str] | list[str] | set[str] | None = None,
        load_relations: Collection[str] | list[str] | set[str] | None = None,
    ) -> list[dict[str, Any]]:
        ...
        Args:
            orm_objects    (Sequence[DeclarativeBase]                    , required): List of ORM objects.
            excludes       (Collection[str] | list[str] | set[str] | None, optional): List or set of attributes to
                                                                                        exclude. Defaults to None.
            load_relations (Collection[str] | list[str] | set[str] | None, optional): List or set of relationships to
                                                                                        include. Defaults to None.
    ...
    def _build_select(
        ...
        Args:
            where          (list[dict[str, Any]] |
                                  dict[str, Any] |
                                              None, optional): List of filter conditions. Defaults to None.
            ...
            order_by       (Collection[str] |
                                  list[str] |
                                   set[str] |
                                        str | None, optional): List or set of order by columns. Defaults to None.
            joins          (Collection[str] |
                                  list[str] |
                                   set[str] | None, optional): List or set of joinable relationships. Defaults to None.
