...
from sqlalchemy.orm import DeclarativeBase, declarative_mixin, Session, joinedload
...
class ReadMixin(BaseMixin):
    ...
    def select_by_where(
        ...
        Args:
            ...
            limit           (int                   , optional): Number of objects to limit. Defaults to 100.
    ...
    def select(
        ...
        Args:
            ...
            joins           (Collection[str] |
                                   list[str] |
                                    set[str] |
                                          None, optional): List or set of joinable relationships. Defaults to None.
            allow_no_result (bool            , optional): Allow no result. Defaults to True.
            warn_mode       (WarnEnum        , optional): Warning mode. Defaults to `WarnEnum.DEBUG`.
            ...
            if joins:
                for _join in joins:
                    if _join and hasattr(cls, _join):
                        ...
                        _options.append(joinedload(getattr(cls, _join)))

            _orm_object = cast(
                DeclarativeBase | Self | None, session.get(cls, id, options=_options)
            )
        except Exception:
            _message = (
