...
from ...exceptions import EmptyValueError, ForeignKeyError, RestrictViolationError
...
def _raise_fk_error(err: DBAPIError) -> None:
    ...
    Raises:
        ...
        RestrictViolationError: If the error is a restrict violation error.
    """
    ...
    elif isinstance(_err_orig, RestrictViolation):
        _err_orig = cast(RestrictViolation, _err_orig)
        _detail = _err_orig.diag.message_detail
        if _detail:
            _detail = _detail.replace("Key ", "").replace('"', "'")
        else:
            _detail = "Restrict violation error occurred!"
        raise RestrictViolationError(_detail)
...
class AsyncDeleteMixin(AsyncReadMixin):
    ...
    async def async_delete_objects(
        ...
        Args:
            ...
            objects       (Sequence[DeclarativeBase] |
