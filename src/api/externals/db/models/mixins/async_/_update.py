...
from sqlalchemy import Update, update, Result, inspect
from sqlalchemy.orm import DeclarativeBase, declarative_mixin, RelationshipProperty
...
class AsyncUpdateMixin(AsyncReadMixin):
    ...
    async def async_update(
        ...
        **kwargs,
        ...
        try:
            _mapper = cast(Mapper, inspect(self.__class__))
            for _key, _val in kwargs.items():
                _prop = _mapper.attrs.get(_key)
                if isinstance(_prop, RelationshipProperty):
                    await getattr(self.awaitable_attrs, _key)  # type: ignore
    ...
    async def async_update_objects(
        cls,
        async_session: AsyncSession,
        orm_objects: Sequence[DeclarativeBase] | list[DeclarativeBase],
        auto_commit: bool = False,
        warn_mode: WarnEnum = WarnEnum.DEBUG,
        **kwargs,
    ) -> list[DeclarativeBase | Self]:
        ...
        Args:
            ...
            orm_objects   (Sequence[DeclarativeBase] |
                                 list[DeclarativeBase], required): List of ORM objects.
            auto_commit   (bool                       , optional): Auto commit. Defaults to False.
            warn_mode     (WarnEnum                   , optional): Warning mode. Defaults to `WarnEnum.DEBUG`.
            **kwargs      (dict[str, Any]             , required): Dictionary of update data.
        ...
        **kwargs,
        ...
        if orm_way:
            _orm_objects = await cls.async_select_by_where(
                async_session=async_session, where=where, limit=0, warn_mode=warn_mode
            )
